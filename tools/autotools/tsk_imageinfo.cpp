...
class TskImageInfo:public TskAuto {
public:
    TskImageInfo(int32_t);
	TskImageInfo(int32_t, bool);
...
TskImageInfo::TskImageInfo(int32_t a_secSkew)
{
    m_curVolAddr = -1;
    m_curPoolVol = -1;
    m_secSkew = a_secSkew;
...
TskImageInfo::TskImageInfo(int32_t a_secSkew, bool a_compute_hash)
{
    m_curVolAddr = -1;
    m_curPoolVol = -1;
    m_secSkew = a_secSkew;
...
uint8_t TskImageInfo::handleError() 
{
...
TSK_RETVAL_ENUM TskImageInfo::processFile(TSK_FS_FILE * /*fs_file*/, const char * /*path*/)
{
    return TSK_OK;
}
...
TSK_FILTER_ENUM
TskImageInfo::filterFs(TSK_FS_INFO * fs_info)
{
...
TSK_FILTER_ENUM
TskImageInfo::filterVol(const TSK_VS_PART_INFO * vs_part)
{
...
TSK_FILTER_ENUM
TskImageInfo::filterPool(const TSK_POOL_INFO * pool_info)
{
...
TSK_FILTER_ENUM
TskImageInfo::filterPoolVol(const TSK_POOL_VOLUME_INFO * pool_vol)
{
...
main(int argc, char **argv1)
{
    ...
    TskImageInfo tskImageInfo(sec_skew, do_hash);
    if (tskImageInfo.openImage(argc - OPTIND, &argv[OPTIND], imgtype,
            ssize)) {
        tsk_error_print(stderr);
        exit(1);
    }
    
    if (tskImageInfo.findFilesInImg()) {
